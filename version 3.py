...
def pie_de_pagina(contenedor):
    tk.Label(contenedor, text="FOODVIDE © 2025 - Todos los derechos reservados", 
...
def pagina_estadisticas(contenedor):
    ...
    def mostrar_dia():
        ...
        tk.Label(contenedor, text="💰 Ventas del Día",
                 font=("Arial", 22, "bold"), bg=COLOR_FONDO, fg=COLOR_AZUL).pack(pady=10)
        tk.Label(contenedor, text="Hoy se vendieron: $1,850.75",
                 font=("Arial", 14), bg=COLOR_FONDO).pack(pady=10)
        pie_de_pagina(contenedor)
    ...
    def mostrar_mensual():
        ...
        tk.Label(contenedor, text="📆 Ventas Mensuales",
                 font=("Arial", 22, "bold"), bg=COLOR_FONDO, fg=COLOR_AZUL).pack(pady=10)
        tk.Label(contenedor, text="Ingresos del mes: $52,340.00",
                 font=("Arial", 14), bg=COLOR_FONDO).pack(pady=10)
        pie_de_pagina(contenedor)

    # -------------------------------------------------------
    # GRÁFICA SIN MATPLOTLIB
    # -------------------------------------------------------
    def mostrar_grafica():
        for w in contenedor.winfo_children(): w.destroy()
        ...
        x = 50
        for v in ventas:
            canvas.create_oval(x - 5, 300 - v * 0.5 - 5,
                               x + 5, 300 - v * 0.5 + 5,
                               fill="purple")
            x += 60
    ...
    # BOTONES
    # -------------------------------------------------------
    tk.Button(contenedor, text="💰 Ventas del Día", font=("Arial", 14),
              bg=COLOR_AZUL, fg="white", width=20, command=mostrar_dia).pack(pady=10)

    tk.Button(contenedor, text="📆 Informe Mensual", font=("Arial", 14),
              bg=COLOR_AZUL, fg="white", width=20, command=mostrar_mensual).pack(pady=10)

    tk.Button(contenedor, text="📈 Gráficas", font=("Arial", 14),
              bg=COLOR_AZUL, fg="white", width=20, command=mostrar_grafica).pack(pady=10)
...
def pagina_perfil(contenedor):
    ...
    tk.Button(contenedor, text="Guardar Cambios", bg=COLOR_AZUL,
              fg="white", font=("Arial", 12), command=guardar).pack(pady=20)
